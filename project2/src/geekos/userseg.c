...
static struct User_Context* Create_User_Context(ulong_t size)  {
	...
	if (UserContext != 0)
		UserContext->memory = Malloc(size);//为核心态进程          
	else
		goto  fail;//内存为空  
	...
	UserContext->dsSelector = Selector(USER_PRIVILEGE, false, 1);//将引用数清0  
 ...
 * data structure.载入一个用户文件进内存并创建User_Context
 * Params:
...
struct User_Context **pUserContext)
{
	...
	int i;
	ulong_t maxva = 0;//需要分配的内存空间
	unsigned numArgs;//进程数目
	ulong_t argBlockSize;//参数块的大小  
	ulong_t size, argBlockAddr;
	struct User_Context *userContext = 0;
	for (i = 0; i < exeFormat->numSegments; ++i) {   //统计segment块所需分配的内存空间
		struct Exe_Segment *segment = &exeFormat->segmentList[i];
		ulong_t topva = segment->startAddress + segment->sizeInMemory;//计算每一块segment的结束位置
		if (topva > maxva)
			maxva = topva;//则最后一个segment结束的位置即为整个segmentList的大小
	}
	Get_Argument_Block_Size(command, &numArgs, &argBlockSize);//获取参数块信息 
	size = Round_Up_To_Page(maxva) + DEFAULT_USER_STACK_SIZE;//用户进程大小=segment块总大小（） + 进程堆栈大小(8192)
	argBlockAddr = size;//??
	size += argBlockSize;//??
...
/*
 * Copy data from user memory into a kernel buffer.将用户态的进程复制到内核缓冲区
 * Params:
...
bool Copy_From_User(void* destInKernel, ulong_t srcInUser, ulong_t bufSize)
{
	...
	if (!Validate_User_Memory(UserContext, srcInUser, bufSize))   
		return false;  //--:user->kernel 
	memcpy(destInKernel, UserContext->memory + srcInUser, bufSize);      
	return true;
}

/*
 * Copy data from kernel memory into a user buffer.将内核态的进程复制到用户态
 * Params:
