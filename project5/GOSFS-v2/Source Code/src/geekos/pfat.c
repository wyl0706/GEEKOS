...
#include <geekos/user.h> // ERROR: User_Context structure
...
static int PFAT_Close(struct File *file)
{
     ...
     */
     // ERROR: I have to fix this to let pfat work with standard syscall
     int i;
...
static int PFAT_Mount(struct Mount_Point *mountPoint)
{
    ...
    /* Create the fake root directory entry. */// what's this fake entry for?
