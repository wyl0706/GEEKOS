...
#include <geekos/user.h> //Detach_User_Context
...
static void Detach_Thread(struct Kernel_Thread* kthread)
{
	...
	// ERROR consider the user context that we didn't release
	//Print("detach\n");
    ...
    struct Kernel_Thread* kthread, struct User_Context* userContext)
{
	...
	// Ïß³Ì¶ÔÏó¹ÒÔØÓÃ»§ÉÏÏÂÎÄ
	// tlocal_key_t key = ;
	Attach_User_Context(kthread, userContext);
	//Print("uC: %d\n", (int)userContext);

	// --------------ERRORCODE
	// ³õÊŒ»¯ÓÃ»§kthreadµÄTLocal
	// ·ñÔòÃ»ÓÐÏú»ÙÓÃ»§¿ÕŒäµÄº¯Êýµ÷ÓÃ
	// Tlocal_Put(key, kthread->tlocalData[key]);
	//--------------ERRORCODE

	// ERROR: žüÐÂÁËuserContext£¬ÔÚuserContextÖÐ±£Žæss
	// ÉèÖÃ¶ÑÕ»¶ÎÃèÊö·û£¬Ñ¡Ôñ×Ó
	// static struct Segment_Descriptor ssDesc;
	// static struct Segment_Descriptor * dsDesc = Allocate_Segment_Descriptor();
	// ssDesc = *desc;
	...
	//	3);
	// ushort_t ssSelector = Selector(3, false, Get_Descriptor_Index(&desc));
	
	// ÉèÖÃÄÚºË¶ÑÕ»Ìø×ªµœµÄÓÃ»§¶ÑÕ»
	...
	// ÉèÖÃESI ÖžÏòargBlock
	kthread->esp += 6*4;
	*((ulong_t *) kthread->esp) = userContext->argBlockAddr;
	kthread->esp -= 6*4;
	//Print("argBA:%u\n", (int)userContext->argBlockAddr);

	// ERROR: no return value return kthread;
}
...
Start_User_Thread(struct User_Context* userContext, bool detached)
{
    ...
    //TODO("Start user thread");
    	struct Kernel_Thread * kthread = Create_Thread(3, detached);
	if(kthread == 0)
	{
		Print("Failed Creating_Thread\n");
		return NULL;
	}
...
void Make_Runnable(struct Kernel_Thread* kthread)
{
    ...
    { int currentQ = kthread->currentReadyQueue;
      ...
      kthread->blocked = false;
      Enqueue_Thread(&s_runQueue[currentQ], kthread); //ERROR[currentQ], kthread);
    }
}
...
//? what decrease the priority of the thread? block and what?
...
void Wait(struct Thread_Queue* waitQueue)
{
    ...
    // ERROR change priority as this thread is blocked;
    current->currentReadyQueue= (current->currentReadyQueue == 0 ? 0 : current->currentReadyQueue--);
