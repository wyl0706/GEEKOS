...
bool Unpack_Path(const char *path, char *prefix, const char **pSuffix)
{
    ...
    // ERROR: we can afford an unslash start
    if (*path == '/')
	//return false;
      ++path;
...
int Close(struct File *file)
{
    ...
    if (rc == 0)
	// ERROR: can't free here;Free(file);
	;
    return rc;
}
...
int Create_Directory(const char *path)
{
    ...
    const char *suffix;
    struct Mount_Point *mountPoint;
    // ERROR: struct File * vnode;

    /* Split path into prefix and suffix */
	// ERROR: if failed, then attempt to create one dir under currentDir
    if (!Unpack_Path(path, prefix, &suffix) && strlen(path)!= 0) 
	strcpy(prefix, path);

	//Print("	prefix:%s\n", prefix);
	//Print("	suffix:%s\n", suffix);
    /* Get mount point for path *///ERROR: need fixing
    // ERROR: if not from rootDir, then we can't find mountPoint; let it be the current DIR's mp
	mountPoint = Lookup_Mount_Point(prefix);
	if (mountPoint == 0) // ERROR: use currentDir as the source of mountPoint
	{
		Print("	mountPoint not found.\n");
		return ENOTFOUND; // no such mountPoint
	}
	else
		Debug("	mountPoint found:%s\n", mountPoint->pathPrefix);
