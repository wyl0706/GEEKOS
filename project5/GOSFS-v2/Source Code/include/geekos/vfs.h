...
// ERROR: to build vnode_List, we have to change this structure.
DEFINE_LIST(VNode_List, File);
...
struct File {
     ...
     */
    // ERROR: VNode link
    DEFINE_LINK(VNode_List, File);
    int mode;			 /* Mode (read vs. write). */
    struct Mount_Point *mountPoint; /* Mounted filesystem file is part of. */
};

IMPLEMENT_LIST(VNode_List, File); // ERROR: Implementation of VNode_List's Ops.
...
// ERROR: static struct File *currentDirVNode;
// ERROR: static struct File *rootDirVNode;
