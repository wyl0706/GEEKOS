...
#include <../include/geekos/ktypes.h>
#include <../include/geekos/segment.h>
#include <../include/geekos/elf.h>
...
#define NUM_USER_LDT_ENTRIES 2
     ...
     */
    struct Segment_Descriptor ldt[NUM_USER_LDT_ENTRIES];//segment descriptor就是段描述符，存在GDT中
    struct Segment_Descriptor* ldtDescriptor;//ldtDescriptor 是LDT的描述Segment Descriptor

    /* The memory space used by the process. */
	char* memory;//memory是用户内存空间的实际地址
    ulong_t size;//Size是用户空间的大小
    ...
    /* Code entry point */
    ulong_t entryAddr;//entryAddr是用户代码的实际地址，进程就是从这个地址开始运行的。
