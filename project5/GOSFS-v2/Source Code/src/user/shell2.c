...
//#define BUFSIZE 79 ERROR: 79 plus 1 is the length of one single line
#define MAX_PATH_LEN 1023
#define BUFSIZE MAX_PATH_LEN + 79
#define DEFAULT_PATH "/c:/a" // this is the environmemtal variable of PFAT
#define GOSFS_ROOT "/d/"
...
//int Patch_Path(char *command); // check if the path is full

/* Maximum number of processes allowed in a pipeline. */
#define MAXPROC 5

int exitCodes = 0;
//static char currentPath[MAX_PATH_LEN] = GOSFS_ROOT;
...
int main(int argc, char **argv)
{
	...
	if(strcmp(procList[0].program, "ls") == 0 || strcmp(procList[0].program, "/c/ls.exe") == 0)
	Print("ls mother fucker!\n");
/*	{
		nproc = Patch_Path(procList[0].command);
		if (nproc < 0){
			Print("ERROR:%d\n", nproc);
			continue;
		}
...
int Patch_Path(char * command)
{
	...
	pstr = Strip_Leading_Whitespace(command);
	if(*pstr == '/')
		pstr++;
	if(*pstr != 'd' )
	{
