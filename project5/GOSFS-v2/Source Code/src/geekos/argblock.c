    ...
    const char *command)
{
    ...
    *(int *)dst = numArgs;
     ...
     */
    *(ulong_t *)dst = (ulong_t)(userAddress + sizeof(struct Argument_Block));
    dst += sizeof(char **);

    /* Argv array immediately follows argv pointer */
    argv = (ulong_t *)dst;
    dst += (numArgs + 1) * sizeof(char *); //ERROR numArgs + 1 skip numArgs char* and NULL 
	...
	if (*s == '\0')
	    break;
	*argv++ = (ulong_t)(userAddress + dst - argBlock);//ERROR(dst - argBlock);  /* Set argv element */
