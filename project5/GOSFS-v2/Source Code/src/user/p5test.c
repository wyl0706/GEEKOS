...
int tStatDirectory()
{
  ...
  Print("rets:%d, isDir:%d\n", (int)retS, (int)s.isDirectory);
  ...
  for (i = 0; i < 50; i++ )
      ...
      for (j = 0; j < 1000; j++ )
	{
	  if (buffer[j] != buffer2[j])
	    {
		Print("j:%d, buf1:%d, buf2:%d\n", j, buffer[j], buffer2[j]);
	      ret = -1;
	      break;
	    }
	}
	  ...
	  for(j = 0; j < 100; j++)
	    {
	      if (buffer2[j] != j)
		{
		Print("buffer2[j-1]=%d, j-1=%d\n", buffer2[j-1], j-1);
		Print("unequal:j=%d, buf2[j]=%d\n", j, buffer2[j]);
		for(j = 0; j < 100; j++)
			Print("buf2[%d]=%d\n", j, buffer2[j]);
		  ret = -1;
		  break;
...
{
  return tWriteReread(36, "/d/file_100k");
}
